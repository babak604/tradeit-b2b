...
export async function POST(req: NextRequest) {
  try {
    ...
    const { data: rpcData, error: rpcError } = await adminSupabase.rpc('cancel_trade_escrow', {
      p_match_id: dealId,
    });
    ...
    if (rpcData && !rpcData.success) {
      return NextResponse.json(
        { error: rpcData.error || 'Cancel escrow failed' },
        { status: 400 }
      );
    }

    return NextResponse.json({ success: true, message: 'Escrow cancelled and refunded' });
  } catch (err: unknown) {
