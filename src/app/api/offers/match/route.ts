...
async function generateEmbedding(text: string): Promise<number[]> {
  if (!openaiApiKey) {
    ...
    return new Array(1536).fill(0).map((_, i) => Math.sin(i + text.length) * 0.05);
  }
...
export async function POST(req: NextRequest) {
  try {
    ...
    const { data: offer, error: fetchErr } = await supabase
      .from('trade_offers')
      .select('id, title, offering_summary, seeking_summary, looking_for_summary, offering_embedding, seeking_embedding')
      .eq('id', offerId)
      .single();
    ...
    const seekingText = offer.seeking_summary || offer.looking_for_summary || 'Required B2B Service';
    const offeringText = offer.offering_summary || offer.title || 'B2B Offering';
    ...
    if (!offeringEmb || !seekingEmb) {
      offeringEmb = await generateEmbedding(`${offer.title || ''}: ${offeringText}`);
      seekingEmb = await generateEmbedding(seekingText);
    ...
    // 3. Execute Reciprocal Matching via Supabase RPC (Aligned parameter keys)
    ...
    if (rpcErr) {
      console.warn('RPC Notice (using fallback if RPC missing):', rpcErr.message);
