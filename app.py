...
def get_ai_deal_appraisal(offered_title, offered_desc, target_title, target_desc):
    ...
    try:
        client = OpenAI(api_key=OPENAI_API_KEY)
        prompt = f"""You are an expert B2B business appraiser. Analyze this barter trade:
        PARTY A OFFERS: {offered_title} ({offered_desc})
        PARTY B OFFERS: {target_title} ({target_desc})
        Provide a 3-paragraph Markdown appraisal: 1) Valuation A 2) Valuation B 3) Mediator's Verdict (Cash Top-Up recommendation)."""
        return client.chat.completions.create(model="gpt-4o-mini", messages=[{"role": "user", "content": prompt}], max_tokens=400, temperature=0.3).choices[0].message.content
...
# -----------------------------------------------------------------------------
# 4. SMART MATCHES & CIRCULAR TRADES (PHASE 14 & 15)
# -----------------------------------------------------------------------------
def render_smart_matches(user_id, my_posts):
    ...
    for my_need in my_needs:
        ...
        if my_need.get("embedding"):
            ...
            except Exception: st.error("Vector search failed. (Check pgvector extension).")
...
def _heuristic_match(need, offer):
    """Fallback logic to detect potential matches for circular trades."""
...
def render_circular_trades(user_id, my_posts):
    ...
    st.caption("Automatically discovers 3-way barter loops: You need B, B needs C, C needs You.")
                                        ...
                                        for c_need in user_needs.get(c_uid, []):
                                            for m_off in my_offers:
                                                if _heuristic_match(c_need, m_off):
                                                    loops.append({
                                                        "A_n": m_need, "A_o": m_off,
                                                        "B_n": b_need, "B_o": b_off, "B_prof": b_off.get("profiles", {}).get("business_name", "Partner B"), "B_uid": b_uid,
                                                        "C_n": c_need, "C_o": c_off, "C_prof": c_off.get("profiles", {}).get("business_name", "Partner C"), "C_uid": c_uid
                                                    })
        if not loops:
            st.info("No 3-way circular loops found in the network right now. Try expanding your categories!")
        else:
            ...
            for idx, loop in enumerate(loops):
                with st.container(border=True):
                    ...
                    if st.button("🤝 Initiate Multi-Party Trade", key=f"loop_{idx}"):
                        # Propose to B (You want B's offer, offering them your need... wait, offer them a placeholder for 3-way)
                        # MVP: Send direct messages to both parties to coordinate
                        st.toast("Multi-party trade initiations coming soon!", icon="🚀")
...
def render_trade_proposal_card(prop, is_incoming, user_id):
    ...
    status, topup = prop.get("status", "pending"), float(prop.get("cash_topup_amount") or 0.0)
...
def main_app():
    ...
    try:
        ...
        is_admin = supabase.table("profiles").select("is_admin").eq("id", user.id).execute().data[0].get("is_admin", False)
    except Exception: pass
    ...
    tabs = st.tabs(["🌐 Feed", "⚡ Semantic Match", "🌀 Circular Trades", "🗺️ Map", "➕ Post", "📋 Listings", f"📬 Inbox ({unread_count})", "🏢 Profile"])
